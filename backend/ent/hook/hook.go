...
// The OperationroomFunc type is an adapter to allow the use of ordinary
// function as Operationroom mutator.
type OperationroomFunc func(context.Context, *ent.OperationroomMutation) (ent.Value, error)

// Mutate calls f(ctx, m).
func (f OperationroomFunc) Mutate(ctx context.Context, m ent.Mutation) (ent.Value, error) {
	mv, ok := m.(*ent.OperationroomMutation)
	if !ok {
		return nil, fmt.Errorf("unexpected mutation type %T. expect *ent.OperationroomMutation", m)
	}
	return f(ctx, mv)
}
