...
// The OperationroomQueryRuleFunc type is an adapter to allow the use of ordinary
// functions as a query rule.
type OperationroomQueryRuleFunc func(context.Context, *ent.OperationroomQuery) error

// EvalQuery return f(ctx, q).
func (f OperationroomQueryRuleFunc) EvalQuery(ctx context.Context, q ent.Query) error {
	if q, ok := q.(*ent.OperationroomQuery); ok {
		return f(ctx, q)
	}
	return Denyf("ent/privacy: unexpected query type %T, expect *ent.OperationroomQuery", q)
}

// The OperationroomMutationRuleFunc type is an adapter to allow the use of ordinary
// functions as a mutation rule.
type OperationroomMutationRuleFunc func(context.Context, *ent.OperationroomMutation) error

// EvalMutation calls f(ctx, m).
func (f OperationroomMutationRuleFunc) EvalMutation(ctx context.Context, m ent.Mutation) error {
	if m, ok := m.(*ent.OperationroomMutation); ok {
		return f(ctx, m)
	}
	return Denyf("ent/privacy: unexpected mutation type %T, expect *ent.OperationroomMutation", m)
}
