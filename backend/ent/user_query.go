...
func (uq *UserQuery) QueryDoctorID() *BookingQuery {
...
func (uq *UserQuery) WithDoctorID(opts ...func(*BookingQuery)) *UserQuery {
	query := &BookingQuery{config: uq.config}
	for _, opt := range opts {
		opt(query)
	}
	uq.withDoctorID = query
	return uq
}
...
func (uq *UserQuery) sqlAll(ctx context.Context) ([]*User, error) {
	...
	if query := uq.withDoctorID; query != nil {
		...
		query.Where(predicate.Booking(func(s *sql.Selector) {
			s.Where(sql.InValues(user.DoctorIDColumn, fks...))
		}))
		...
		for _, n := range neighbors {
			fk := n.user_doctor_id
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "user_doctor_id" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "user_doctor_id" returned %v for node %v`, *fk, n.ID)
			}
			node.Edges.DoctorID = append(node.Edges.DoctorID, n)
		}
	}
