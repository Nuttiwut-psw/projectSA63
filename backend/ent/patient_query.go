...
func (pq *PatientQuery) QueryPatientID() *BookingQuery {
...
func (pq *PatientQuery) WithPatientID(opts ...func(*BookingQuery)) *PatientQuery {
	query := &BookingQuery{config: pq.config}
	for _, opt := range opts {
		opt(query)
	}
	pq.withPatientID = query
	return pq
}
...
func (pq *PatientQuery) sqlAll(ctx context.Context) ([]*Patient, error) {
	...
	if query := pq.withPatientID; query != nil {
		...
		query.Where(predicate.Booking(func(s *sql.Selector) {
			s.Where(sql.InValues(patient.PatientIDColumn, fks...))
		}))
		...
		for _, n := range neighbors {
			fk := n.patient_patient_id
			if fk == nil {
				return nil, fmt.Errorf(`foreign-key "patient_patient_id" is nil for node %v`, n.ID)
			}
			node, ok := nodeids[*fk]
			if !ok {
				return nil, fmt.Errorf(`unexpected foreign-key "patient_patient_id" returned %v for node %v`, *fk, n.ID)
			}
			node.Edges.PatientID = append(node.Edges.PatientID, n)
		}
	}
