...
func (pc *PatientCreate) AddPatientIDIDs(ids ...int) *PatientCreate {
	pc.mutation.AddPatientIDIDs(ids...)
	return pc
}

// AddPatientID adds the patient_id edges to Booking.
func (pc *PatientCreate) AddPatientID(b ...*Booking) *PatientCreate {
	ids := make([]int, len(b))
	for i := range b {
		ids[i] = b[i].ID
	}
	return pc.AddPatientIDIDs(ids...)
}
...
func (pc *PatientCreate) createSpec() (*Patient, *sqlgraph.CreateSpec) {
	...
	if nodes := pc.mutation.PatientIDIDs(); len(nodes) > 0 {
